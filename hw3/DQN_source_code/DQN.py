...
parser.add_argument("--train_ep", default=1200, type=int)
...
parser.add_argument(
    "--save_dir", default="/content/drive/My Drive/colab/model3")
# you can plot the figure accroding to the file
parser.add_argument(
    "--log_file", default="/content/drive/My Drive/colab/log3.txt")
...
class DQN(object):
    ...
    def evaluate_action(self, state, rand=0):
        if random.random() < rand:
            ...
            act = torch.tensor([random.sample([0, 0, 0, 1, 1, 1, 1, 1, 1, 2], 1)], device=device, dtype=torch.long)
            return act, None 

        with torch.no_grad():
            q = self.target_net(state)
            act = q.max(1)[1].view(1, 1)
            return act, q
    ...
    for i_episode in range(num_episodes):
        ...
        if i_episode % args.eval_per_ep == 0:
            ...
            for t_ep in range(test_times):
                ...
                for _ in range(10000):
                    action, q = agent.evaluate_action(state)
...
def test():
    try:
        os.mkdir("images")
        os.mkdir("images/act0")
        os.mkdir("images/act1")
        os.mkdir("images/act2")
    except:
        pass
    ...
    for i_episode in range(10):
        ...
        for i in range(10000):
            ...
            action, q = agent.evaluate_action(state, test_epsilon)
            if i % 200 == 0 and img is not None:
                q = q.detach().cpu().numpy()[0]
                plt.title('Noop: %.3f, Up: %.3f, Down: %.3f\naverage: %.3f' %
                          (q[0], q[1], q[2],np.mean(q)))
                plt.imshow(img)
                plt.savefig('images/act%d/epi%d_step%d.png' %
                            (np.argmax(q), i_episode, i), bbox_inches='tight')
                plt.close()

            next_state, reward, done, img = env.step(action.item())
